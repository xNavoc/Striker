  ...
  if team_id:
    try:
      ...
      bp['bp_hr9'] = min(
          ...
          max(
              0.4,
              round((max(4.0, hrs * 0.36) * 9.0) / max(40.0, ip * 0.38), 2),
          ),
      )
...
def fetch_pitcher_profile(pitcher_id: int, pitcher_name: str):
  ...
  hr9_overall, whip_overall, era_overall, season_ip, games_started = (
      1.20,
      1.25,
      4.20,
      0.0,
  ...
  hr9_lhb, hr9_rhb, baa_lhb, baa_rhb, slg_lhb, slg_rhb = (
      None,
      None,
      None,
      None,
  ...
  is_bg = (
      pos == 'RP'
      or 'TBD' in pitcher_name
      or 'Bullpen' in pitcher_name
      or (games_started > 0 and avg_ip < 3.0)
  )

  if season_ip >= 30.0:
    eff_hr9, eff_whip, eff_k9, eff_baa, eff_slg = (
        hr9_overall,
        whip_overall,
        k9_overall,
        baa_overall,
    ...
    eff_hr9 = round(
        ((season_ip * hr9_overall) + (30.0 * 1.20)) / (season_ip + 30.0), 2
    )
    eff_whip = round(
        ((season_ip * whip_overall) + (30.0 * 1.25)) / (season_ip + 30.0), 2
    )
    eff_k9 = round(
        ((season_ip * k9_overall) + (30.0 * 8.50)) / (season_ip + 30.0), 2
    )
    eff_baa = round(
        ((season_ip * baa_overall) + (30.0 * 0.245)) / (season_ip + 30.0), 3
    )
    eff_slg = round(
        ((season_ip * slg_overall) + (30.0 * 0.405)) / (season_ip + 30.0), 3
    )
  ...
  prof = {
      ...
      'p_hand': p_hand,
      'hr9': eff_hr9,
      'hr9_lhb': hr9_lhb or eff_hr9,
      'hr9_rhb': hr9_rhb or eff_hr9,
      'baa': eff_baa,
      'baa_lhb': baa_lhb or eff_baa,
      'baa_rhb': baa_rhb or eff_baa,
      'slg': eff_slg,
      'slg_lhb': slg_lhb or eff_slg,
      'slg_rhb': slg_rhb or eff_slg,
      'k9': eff_k9,
      'whip': eff_whip,
      'avg_ip': avg_ip,
      'badge': badge,
...
def fetch_batter_profile(person_id: int, b_name: str, p_hand: str):
  ...
  raw_iso, raw_ba, raw_obp, raw_slg, raw_hr, season_pa = (
      0.080,
      0.220,
      0.290,
      0.300,
  ...
  sp_iso, sp_ba, sp_obp, sp_slg, sp_hr, sp_pa = (
      None,
      None,
      None,
      None,
  ...
  if season_pa >= 80:
    ...
    eff_iso = round(
        ((season_pa * raw_iso) + (100.0 * 0.155)) / (season_pa + 100.0), 3
    )
    eff_ba = round(
        ((season_pa * raw_ba) + (100.0 * 0.245)) / (season_pa + 100.0), 3
    )
    eff_obp = round(
        ((season_pa * raw_obp) + (100.0 * 0.315)) / (season_pa + 100.0), 3
    )
    eff_slg = round(
        ((season_pa * raw_slg) + (100.0 * 0.400)) / (season_pa + 100.0), 3
    )
    eff_hr_pa = raw_hr / max(1.0, float(season_pa))
  else:
    eff_iso, eff_ba, eff_obp, eff_slg, eff_hr_pa = (
        0.080,
        0.220,
        0.285,
        0.300,
  ...
  final_hr_pa = (
      (sp_hr / float(sp_pa))
      if (sp_hr is not None and sp_pa >= 35)
      else eff_hr_pa
  )
  ...
  split_desc = (
      '⚡ Rev Adv'
      if (same_hand and (final_iso >= 0.220 or final_ba >= 0.280))
      else ('🛡️ Same-Hand' if same_hand else '🔥 Platoon')
  )
...
def fetch_projected_lineup_rotowire(team_name: str):
  try:
    ...
    matches = re.findall(
        rf'{team_name}.*?lineup__list">(.*?)</ul>', response.text, re.DOTALL
    )
    if matches:
...
def load_daily_slate(target_date_str=None):
  ...
  print(
      f'[i] Loading Unified Slate Data for {today_str} (Season'
      f' {CURRENT_SEASON})...'
  )
  ...
  upcoming = (
      ...
      if target_date_str
          ...
          for g in schedule
          if not any(
              x in g.get('status', 'Scheduled')
              for x in ['Final', 'Postponed', 'Cancelled']
          )
      ]
  )
  ...
  for g in upcoming:
    ...
    weather = g.get('weather', {}).get('condition', 'Dome / Standard')
    ...
    away_p_prof = fetch_pitcher_profile(
        g.get('away_probable_pitcher_id', 0), away_p_name
    )
    home_p_prof = fetch_pitcher_profile(
        g.get('home_probable_pitcher_id', 0), home_p_name
    )
    ...
    away_batters = [
        (
            ...
            b_id,
            name,
            fetch_batter_profile(b_id, name, home_p_prof['p_hand']),
        )
        for order, b_id, name in get_team_top6(
            'away', away_team, away_id, box
        )
    ]
    home_batters = [
        (
            ...
            b_id,
            name,
            fetch_batter_profile(b_id, name, away_p_prof['p_hand']),
        )
        for order, b_id, name in get_team_top6(
            'home', home_team, home_id, box
        )
    ]
