...
BALLPARK_FACTORS = {
    'Great American Ball Park': {'hr': 128, 'hits': 105, 'tb': 114, 'overall': 116, 'k': 96},
    'Coors Field':              {'hr': 122, 'hits': 118, 'tb': 124, 'overall': 120, 'k': 88},
    'Yankee Stadium':           {'hr': 118, 'hits': 99,  'tb': 106, 'overall': 108, 'k': 102},
    'Fenway Park':              {'hr': 112, 'hits': 108, 'tb': 112, 'overall': 110, 'k': 97},
    'Citizens Bank Park':       {'hr': 115, 'hits': 102, 'tb': 107, 'overall': 108, 'k': 101},
    'Guaranteed Rate Field':    {'hr': 111, 'hits': 101, 'tb': 104, 'overall': 106, 'k': 100},
    'Camden Yards':             {'hr': 105, 'hits': 100, 'tb': 101, 'overall': 102, 'k': 101},
    'Dodger Stadium':           {'hr': 114, 'hits': 98,  'tb': 104, 'overall': 106, 'k': 103},
    ...
    'Target Field':             {'hr': 95,  'hits': 99,  'tb': 97,  'overall': 97,  'k': 101},
    'PNC Park':                 {'hr': 90,  'hits': 98,  'tb': 94,  'overall': 94,  'k': 102},
    'Kauffman Stadium':         {'hr': 92,  'hits': 104, 'tb': 98,  'overall': 98,  'k': 95},
...
def load_daily_slate(target_date_str=None):
    ...
    for g in upcoming:
        ...
        def get_team_top6(side_key, team_id):
                ...
                try:
                    ...
                    for line in roster.strip().split('\n'):
                        ...
                        if len(parts) >= 3 and parts[1] not in ['P', 'RHP', 'LHP']:
                            ...
                            p_name = " ".join(parts[2:])
                            if p_name not in [v[2] for v in valid]:
                                valid.append((len(valid) + 1, p_id, p_name))
                                if len(valid) == 6: break
                except Exception:
                    pass
            return valid

        away_batters = [(order, b_id, name, fetch_batter_profile(b_id, name, home_p_prof['p_hand'])) for order, b_id, name in get_team_top6('away', away_id)]
        home_batters = [(order, b_id, name, fetch_batter_profile(b_id, name, away_p_prof['p_hand'])) for order, b_id, name in get_team_top6('home', home_id)]

        games_data.append({
            'game_id': gid, 'venue': venue, 'park_factors': park, 'weather': weather,
